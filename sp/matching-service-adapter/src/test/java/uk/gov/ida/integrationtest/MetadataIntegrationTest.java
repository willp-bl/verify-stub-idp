...
public class MetadataIntegrationTest {
    ...
    public static final MatchingServiceAdapterAppExtension applicationRule = new MatchingServiceAdapterAppExtension(
            Map.ofEntries(
                    Map.entry("signingKeys.primary.publicKey.name", MSA_SIGNING_PRIMARY),
                    Map.entry("signingKeys.secondary.publicKey.name", MSA_SIGNING_SECONDARY),
                    Map.entry("encryptionKeys[0].publicKey.name", MSA_ENCRYPTION_PRIMARY)
            ));
