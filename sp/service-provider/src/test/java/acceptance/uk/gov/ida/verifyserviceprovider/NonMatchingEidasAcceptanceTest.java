...
import acceptance.uk.gov.ida.verifyserviceprovider.rules.NonMatchingVerifyServiceProviderAppRule;
...
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.aHubResponseContainingEidasUnsignedAssertions;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.aValidEidasResponse;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.anEidasResponseIssuedByACountry;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.anEidasResponseIssuedByACountryWithUnsignedAssertions;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.anInvalidAssertionSignatureEidasResponse;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.anInvalidSignatureEidasResponseIssuedByACountry;
import static acceptance.uk.gov.ida.verifyserviceprovider.builders.AssertionHelper.getReEncryptedKeys;
...
import static stubidp.test.devpki.TestEntityIds.STUB_COUNTRY_TWO;
...
public class NonMatchingEidasAcceptanceTest {
    ...
    private final XmlObjectToBase64EncodedStringTransformer<org.opensaml.saml.saml2.core.Response> xmlToB64Transformer = new XmlObjectToBase64EncodedStringTransformer<>();
    ...
    public void shouldMapAttributesCorrectly() throws Exception {
        ...
        String base64Response = xmlToB64Transformer.apply(samlResponse);
        Response response = appWithEidasEnabled.client().target(format("http://localhost:%s/translate-response", appWithEidasEnabled.getLocalPort())).request().post(
                Entity.json(new TranslateSamlResponseBody(base64Response, "requestId", LEVEL_2, null))
        );
