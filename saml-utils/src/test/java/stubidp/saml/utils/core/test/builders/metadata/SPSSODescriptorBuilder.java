...
import static stubidp.saml.utils.core.test.builders.metadata.KeyDescriptorBuilder.aKeyDescriptor;
import static stubidp.saml.utils.core.test.builders.metadata.X509CertificateBuilder.aX509Certificate;
...
public class SPSSODescriptorBuilder {
    ...
    private KeyDescriptor defaultSigningKeyDescriptor = aKeyDescriptor().withKeyInfo(KeyInfoBuilder.aKeyInfo().withKeyName(TestEntityIds.HUB_ENTITY_ID).withX509Data(aX509Data().withX509Certificate(aX509Certificate().build()).build()).build()).withUse(UsageType.SIGNING.toString()).build();
    private KeyDescriptor defaultEncryptionKeyDescriptor = aKeyDescriptor().withKeyInfo(KeyInfoBuilder.aKeyInfo().withKeyName(TestEntityIds.HUB_ENTITY_ID).withX509Data(aX509Data().withX509Certificate(aX509Certificate().build()).build()).build()).withUse(UsageType.ENCRYPTION.toString()).build();
