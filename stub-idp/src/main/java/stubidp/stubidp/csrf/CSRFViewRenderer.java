    ...
    public void render(View view, Locale locale, OutputStream output) throws IOException {
        ...
        if(view instanceof IdpPageView && ((IdpPageView)view).getCsrfToken().isPresent()) {
            ...
            for (int i = 0; i < nodeList.size(); i++) {
                ...
                if (item.children().stream().filter(it -> (it.tag().getName().equals("input") && it.hasAttr("name") && it.attr("name").equals(CSRFCheckProtectionFilter.CSRF_PROTECT_FORM_KEY))).count() > 0) {
                    throw new CSRFConflictingFormAttributeException();
                } else {
                    item.appendChild(new Element(Tag.valueOf("input"), "", new Attributes() {{
                        put("name", CSRFCheckProtectionFilter.CSRF_PROTECT_FORM_KEY);
                        put("id", CSRFCheckProtectionFilter.CSRF_PROTECT_FORM_KEY);
